...
type ClientI interface {
	...
	NewAccount(acc local_data.AccountReader) error
	CheckProxyStatus() error
	InitHttpClient() error
	Login() (*software_user.User, error)
	SetCliState(cliState int)
	GetAccount() local_data.AccountReader
	ToMap() map[string]interface{}
	start() (*software_user.User, error)
	Stop() error
	Delete() error
	SetAccount(account local_data.AccountReader) error
}

func (b *BaseClient) GetAccount() local_data.AccountReader {
	return b.Account
}
func (b *BaseClient) GetID() uint {
...
func (b *BaseClient) Stop() error {
	go func() {
		...
		if err := b.stop(); err != nil {
			b.Error("停止账号错误", zap.Error(err))
		}
	}()
	return nil
}
func (b *BaseClient) Delete() error {
	go func() {
		l := b.StartLock.TryLock()
		if l {
		...
		if err := global.GVA_DB.Delete(b.Account).Error; err != nil {
			b.Error("删除账号错误", zap.Error(err))
		}
	}()
	return nil
}
...
func (b *BaseClient) SetAccount(account local_data.AccountReader) error {
	had := false

	if len(account.GetPassword()) > 0 {
		b.Account.SetPassword(account.GetPassword())
		had = true
	}
	if account.GetProxyID() > 0 {
		var tempProxy fifa.FIFAProxy
		err := global.GVA_DB.Model(&fifa.FIFAProxy{}).
			Where(&fifa.FIFAProxy{
				Id:     account.GetProxyID(),
				UserID: b.Account.GetUserID(),
			}).First(&tempProxy).Error
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return fmt.Errorf("代理%d不存在", account.GetProxyID())
		} else if err != nil {
			return fmt.Errorf("获取代理错误，原因：%w", err)
		}
		b.Account.SetProxyID(account.GetProxyID())
		b.Proxy = &tempProxy
		had = true
	}
	if !had {
...
func (b *BaseClient) ToMap() map[string]interface{} {
	m := map[string]interface{}{}
	m["ID"] = b.Account.GetID()
	m["Cookie"] = b.Account.GetCookie()
	m["proxy_id"] = b.Account.GetProxyID()
	m["user_name"] = b.Account.GetUserName()
	m["password"] = b.Account.GetPassword()
	m["user_id"] = b.Account.GetUserID()
	m["platform"] = b.Account.GetPlatform()
	m["comment"] = b.Comment
	m["is_running"] = b.IsRunning()
	return m
}
